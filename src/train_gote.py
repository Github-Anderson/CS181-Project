...
def train(episodes=150, save_interval=15):  # 增加训练轮数
    # 修改：创建先手对手和后手agent
    opponent = MinimaxPlayer("RED", depth = 2)
    agent = Neural_ApproximateQLearningPlayer("BLUE")  # BLUE为后手
    ...
    # 创建保存目录
    os.makedirs("models_gote", exist_ok=True)
    ...
    for episode in tqdm(range(episodes)):
        ...
        # 计算胜率(每100轮计算一次)
        if (episode+1) % 100 == 0:
            start_idx = max(0, episode-99)
            recent_wins = sum(1 for i in range(start_idx, episode+1)
                            if rewards_history[i] > 0)
            win_rate = recent_wins / (episode - start_idx + 1)
        ...
        if (episode+1) % save_interval == 0:
            agent.save_model(f"models_gote/gote_agent_ep{episode+1}.pth")
    ...
    plt.title('Rewards (Gote)')
    
    plt.subplot(132)
    plt.plot(win_rate_history)
    plt.title('Win Rate (Gote)')
    
    plt.subplot(133)
    plt.plot(steps_history)
    plt.title('Steps per Episode (Gote)')
    
    plt.tight_layout()
    plt.savefig('training_curves_gote.png')
    plt.close()
